...
- `join`:主要讲两个文件有相同数据的一行,相同字段放在前面
    ...
    **[参数]**
    ...
    将`etc/passwd`按`：`分隔的第4个字段 与 `etc/grou`p的第3个字段比较，如果相同，则将他两同行数据放在一起
